...
## Latency
- **Mean Latency**: **107.49 ms** per image.
- **Profiling Device**: Intel64 Family 6 Model 154 Stepping 3 CPU (laptop run under Windows 11).
- **Latency Breakdown**:
  - Image Loading/Decoding: **~72 ms** (representing 67.0% of runtime).
  - Feature Extraction (FFT, color stats, blockiness, etc.): **~35 ms** (representing 32.5% of runtime).
...
- **Cloud Server**: **$0.02 per 1,000 images** ($20.00 per million images).
  - *Assumptions*: A small cloud instance (e.g., AWS `t3.medium` at ~$0.0416/hr) can serve sequentially at 107.49 ms/image (~33,490 images/hr). Raw compute cost is $1.24 per million images. Factoring in a 15x overhead margin for load balancing, network latency, and server idling, the final estimate is $20.00 per million images.
